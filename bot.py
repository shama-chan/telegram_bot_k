...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- если ожидается отзыв ---
    if step == "feedback_comment":
        user_text = text.strip()

        # блокируем кнопки меню во время отзыва
        if user_text in ["🆕 Создать тикет", "📂 Мои тикеты", "📖 FAQ", "⚙️ Изменить данные"]:
            await update.message.reply_text("❗ Сначала оставьте отзыв текстом 👇")
            return

        if not user_text:
            await update.message.reply_text("❗ Пожалуйста, напишите отзыв текстом 👇")
            return
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO feedback (ticket_id, user_id, stars, comment, created_at) VALUES (?, ?, ?, ?, ?)",
            (tid, update.effective_user.id, stars, user_text, datetime.utcnow().isoformat())
        )
        conn.commit()
        conn.close()
        ...
        stars_display = "⭐️" * stars

        report_text = (
            f"🎫 Тикет #{tid}\n"
            f"👤 {u['full_name'] if u else 'Unknown'}\n"
            f"💬 {t['description'] if t else ''}\n"
            f"📌 Статус: Закрыт админом {t['assigned_to'] if t and t['assigned_to'] else '—'} ✅\n"
            f"📈 Оценка: {stars_display}\n"
            f"📄 Отзыв: {user_text}"
        )
        try:
            await context.bot.send_message(CHANNEL_ID, report_text)
        except Exception as e:
            logger.error(f"Не удалось отправить отзыв в канал: {e}")

        context.user_data.clear()
        return

    # --- если ожидаются ЗВЁЗДЫ ---
    if step == "waiting_feedback":
        # блокируем любые кнопки, пока не выбраны звёзды
        await update.message.reply_text("❗ Пожалуйста, выберите оценку (звёзды) ниже 👇")
        return

    # === Если нажата кнопка меню — только если нет активного шага ===
    if text in ["🆕 Создать тикет", "📂 Мои тикеты", "📖 FAQ", "⚙️ Изменить данные"] and step is None:
        context.user_data.clear()
    ...
    # --- шаг редактирования ФИО
    if step == "edit_full_name":
        context.user_data["new_full_name"] = text.strip()
        context.user_data["step"] = "edit_place"
        await update.message.reply_text(
            "Теперь введите новое место (этаж, кабинет и т.д.):"
        )
        return

    # --- шаг редактирования места
    if step == "edit_place":
        new_place = text.strip()
        new_full_name = context.user_data.get("new_full_name")

        if not new_full_name:
            await update.message.reply_text(
                "Ошибка: имя не указано. Попробуйте снова через 'Изменить данные'."
            )
            context.user_data.clear()
            return
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("close_"):
        ...
        try:
            ...
            # --- отправка кнопок для оценки ---
            kb_feedback = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("⭐️", callback_data=f"rate_{tid}_1"),
                    InlineKeyboardButton("⭐️⭐️", callback_data=f"rate_{tid}_2"),
                    InlineKeyboardButton("⭐️⭐️⭐️", callback_data=f"rate_{tid}_3"),
                    InlineKeyboardButton("⭐️⭐️⭐️⭐️", callback_data=f"rate_{tid}_4"),
                    InlineKeyboardButton("⭐️⭐️⭐️⭐️⭐️", callback_data=f"rate_{tid}_5"),
                ]
            ])
            await context.bot.send_message(
                t["user_id"],
                f"🙏 Пожалуйста, оцените работу администратора по тикету #{tid}:",
                reply_markup=kb_feedback
            )
            context.user_data["step"] = "waiting_feedback"
            context.user_data["feedback_ticket"] = tid

        except Exception as e:
            logger.error(f"Не удалось уведомить пользователя {t['user_id']}: {e}")

# ----------------- Создание тикета из user_data -----------------
